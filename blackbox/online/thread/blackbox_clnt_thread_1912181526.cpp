...
//	void* thread_return;
	...
	while(1)
	{
		...
		//thread
		if((thread_id = pthread_create(&snd_thread, NULL, send_data, (void*)&DS)) < 0)
		{
			perror("thread err: ");
			exit(1);
		}
		pthread_join(snd_thread, (void**)&status);

		//send file_name
//		send(DS->clnt_sock, DS->file_name, strlen(DS->file_name) + 1, 0);
//		cout <<"** send file_name: " << DS->file_name << endl;
//
//		if((DS->fd = fopen(DS->save_path, "rb")) == NULL)
//			perror("fopen err: ");
//
//		while(1)
//		{
//			DS->file_len = fread(DS->buf_snd, 1, sizeof(DS->buf_snd), DS->fd);
//			//send data
